...
Phase 2 agents:
  tumor_analysis_node → similarity → clinical_reasoning_node → report

Phase 3 agents (appended after Phase 2):
  patient_memory → hitl_validation → cap_report_node → visualize

State flows end-to-end from Phase 1 output through all agents.

NOTE: LangGraph does not allow a node name to be identical to a state
key. The three nodes that previously clashed with state keys have been
renamed with a "_node" suffix:
  "tumor_analysis"   → "tumor_analysis_node"
  "clinical_reasoning" → "clinical_reasoning_node"
  "cap_report"       → "cap_report_node"
"""
...
class Phase2State(TypedDict):
    ...
    tumor_analysis: Dict[str, Any]        # state key — holds the analysis result
    clinical_reasoning: Optional[str]     # state key — holds the reasoning text
    report_path: Optional[str]

    # ── Phase 3 ──
    patient_history: List[Dict[str, Any]]
    physician_corrections: Dict[str, Any]
    cap_report: Dict[str, Any]            # state key — holds the CAP report data
...
def build_phase2_pipeline():
    ...
    # Node names use _node suffix where they would otherwise clash with state keys.
    workflow.add_node("tumor_analysis_node", run_tumor_analysis)
    workflow.add_node("similarity", run_similarity_agent)
    workflow.add_node("clinical_reasoning_node", run_clinical_reasoning)
    workflow.add_node("report", run_report_generation)

    workflow.add_edge(START, "tumor_analysis_node")
    workflow.add_edge("tumor_analysis_node", "similarity")
    workflow.add_edge("similarity", "clinical_reasoning_node")
    workflow.add_edge("clinical_reasoning_node", "report")
...
def build_phase3_pipeline():
    ...
    workflow.add_node("cap_report_node", run_cap_reporting)
    ...
    workflow.add_edge("hitl_validation", "cap_report_node")
    workflow.add_edge("cap_report_node", "visualize")
